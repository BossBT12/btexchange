...
export default function TwoFactorAuthPage() {
  ...
  const [setupData, setSetupData] = useState(null); // { secret, qrCodeUrl, qrCodeDataUrl }
    ...
    try {
      ...
      if (response?.success && response?.data) {
        ...
        setError(
          response?.message ??
            t(
              "twoFactor.errors.startSetupFailed",
  ...
  const handleVerifyAndEnable = async (e) => {
    ...
    if (!token || token.length !== 6) {
      setError(
        t(
          "twoFactor.errors.invalidCodeShort",
    ...
    if (!setupData?.secret) {
      setError(
        t(
          "twoFactor.errors.missingSetupData",
          "Setup data missing. Please start setup again."
        )
      );
      return;
    }
    try {
      ...
      setError(null);
      setSuccess(null);
      const response = await authService.verifyAndEnable2FA({
  ...
  const handleDisable = async (e) => {
    ...
    if (!token || token.length !== 6) {
      setError(
        t(
          "twoFactor.errors.disableCodeRequired",
          "Please enter your current 6-digit 2FA code to disable."
        )
      );
      return;
    }
    try {
      ...
      setError(
        err?.message ??
          err?.data?.message ??
          t(
            "twoFactor.errors.disableFailed",
  ...
  const handleCopySecret = async () => {
    if (setupData?.secret) {
      await copyToClipboard(setupData.secret, setCopied);
    }
  };
    ...
    >
      ...
      <Box sx={{ px: 2, pt: SPACING.MD }}>
        ...
        >
          {t(
            "twoFactor.intro",
            "Add an extra layer of security to your account. When enabled, you will need your password and a code from an authenticator app to sign in or perform sensitive actions."
          )}
        </Typography>
          ...
          <Box>
            {!setupData ? (
              ...
              >
                ...
                >
                {setupLoading
                  ? t("twoFactor.actions.loading", "Loading...")
                  : t(
                      "twoFactor.actions.enable",
              ...
              >
                ...
                >
                  {t(
                    "twoFactor.setup.scanDescription",
                ...
                >
                  {(setupData.qrCodeDataUrl || setupData.qrCodeUrl) ? (
                    <Box
                      component="img"
                      src={setupData.qrCodeDataUrl || setupData.qrCodeUrl}
                ...
                <Box component="form" onSubmit={handleVerifyAndEnable}>
                  <TextField
                    ...
                    placeholder={t(
                      "twoFactor.setup.codePlaceholder",
                      "000000"
                    )}
                  ...
                  <Box sx={{ display: "flex", gap: 1 }}>
                    ...
                    >
                      {verifyLoading ? (
                        ...
                        t(
                          "twoFactor.actions.verifyEnable",
                          "Verify and enable"
                        )
                      )}
                    </Button>
          ...
          >
            ...
            >
              {t(
                ...
                "Two-factor authentication is protecting your account. To disable it, enter your current 2FA code below."
              )}
            </Typography>
            <Box component="form" onSubmit={handleDisable}>
              <TextField
                label={t(
                  "twoFactor.enabled.codeLabel",
                ...
                placeholder={t(
                  "twoFactor.enabled.codePlaceholder",
                  "000000"
                )}
                inputProps={{ maxLength: 6 }}
                fullWidth
