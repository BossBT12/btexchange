...
import { Box, Table, TableBody, TableRow, TableCell, Typography, TableHead } from '@mui/material'
...
const PriceTableRow = memo(({ coin, onRowClick }) => {
    const formattedPrice = useMemo(() => {
        return coin?.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }, [coin?.price]);
    ...
    return (
        ...
        >
            <TableCell>
                <Typography variant="body1" sx={{ color: AppColors.TXT_MAIN, fontWeight: "bold", lineHeight: 1 }}>
                    {formatPairForDisplay(coin.pair)}
                </Typography>
                <Typography variant="caption" sx={{ color: AppColors.TXT_SUB }}>{formatCompact(coin.volume24h)}</Typography>
            </TableCell>
            <TableCell align="left">
                <Typography variant="body1" sx={{ color: AppColors.TXT_MAIN, lineHeight: 1 }}>
                    {formattedPrice}
                </Typography>
...
const LandingPageList = ({ limit = TOP_ROWS_LIMIT, showAll = false, searchValue = "", activeTab = "" }) => {
    ...
    const displayPrices = useMemo(() => {
        ...
        if (normalizedSearch) {
            result = result.filter((p) => {
                ...
                const display = formatPairForDisplay(p?.pair ?? "").toLowerCase().replace(/\s/g, "");
                return raw.includes(normalizedSearch) || display.includes(normalizedSearch);
            });
        }
    ...
    useEffect(() => {
        const tradeChagne = async () => {
            ...
            await api.pairPrices((prices) => {
                setMemoizedPrices(prices?.map((price) => ({
                    ...price,
                    changePercent: price?.open24h > 0 ? ((price?.price - price?.open24h) / price?.open24h) * 100 : 0,
                })) || []);
            });
        }
        if (isConnected) {
                ...
                {memoizedPrices?.length === 0 ? (
                    ...
                    displayPrices?.map((coin) => (
                        <PriceTableRow
                            key={coin.pair}
                            coin={coin}
                            onRowClick={() => navigate("/trade", { state: { selectedPair: coin.pair } })}
                        />
                    )))}
            </TableBody>
        </Table>
